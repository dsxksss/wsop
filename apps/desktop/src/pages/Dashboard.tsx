...
import { Building2, Activity, CheckCircle2, Boxes } from "lucide-react";
...
export default function Dashboard() {
  ...
  const deployments = customers.data?.reduce((sum, c) => sum + c.active_deployments, 0);
  ...
  const kpis = [
    ...
    { label: "在用部署实例", value: deployments, icon: Boxes },
  ];
  ...
  return (
    <div className="p-6 md:p-8 flex flex-col gap-6">
      <div>
        ...
        <p className="text-xs text-zinc-400">私有化部署客户维护总览</p>
      </div>
        ...
        {kpis.map((kpi) => (
          <div key={kpi.label} className="card p-5">
            <div className="flex items-center justify-between mb-3">
              <span className="meta-label-tag text-zinc-500">{kpi.label}</span>
              <kpi.icon size={16} className="text-emerald-400/70" />
            </div>
      ...
      <div className="card p-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col divide-y divide-zinc-800/50">
            {records.data.slice(0, 6).map((r) => (
              <div key={r.id} className="flex items-center gap-3 py-2.5 text-xs">
                ...
                />
                <span className="text-zinc-200 font-medium truncate flex-1">{r.title}</span>
                <span className="text-zinc-500 truncate hidden sm:block">{r.customer_name}</span>
                <span className="text-zinc-600 font-mono-data shrink-0">
          ...
          <div className="text-xs text-zinc-500">暂无维护记录</div>
        )}
      </div>
    </div>
  );
